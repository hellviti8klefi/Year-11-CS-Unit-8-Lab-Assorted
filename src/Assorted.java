...
public class Assorted {
    ...
    public static int findSum(List<?> list) {
        ...
        for (Object item : list) {
            if (item instanceof Integer) {
                ...
                } catch (NumberFormatException e) { // non-INT str
                }
            }
        }
        return sum;
    ...
    public static int busStop(List<Integer[]> list) {
        ...
        for (Integer[] stop : list) {
            peopleOnBus += stop[0]; // on
            peopleOnBus -= stop[1]; // off
        }
        return peopleOnBus;
    }
    ...
    public static List<Integer> sortOdd(List<Integer> list) {
        // Collect odd numbers into a list
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for (int i = Math.max(1, lowerBound); i <= upperBound; i++) {
            String numStr = Integer.toString(i);
            int sum = 0;

            for (int j = numStr.length() - 1, position = 1; j >= 0; j--, position++) {
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        ...
        for (Integer motif : list) {
            if (motif >= 0 && motif < counts.length) {
                if (counts[motif] < n) {
                    result.add(motif);
                    counts[motif]++;
                }
    ...
    public static List<String> wildWest(List<String> directions) {
        ...
        for (String direction : directions) {
            if (result.isEmpty() ||
                    !((direction.equals("NORTH") && result.get(result.size() - 1).equals("SOUTH")) ||
                            (direction.equals("SOUTH") && result.get(result.size() - 1).equals("NORTH")) ||
                            (direction.equals("EAST") && result.get(result.size() - 1).equals("WEST")) ||
                            (direction.equals("WEST") && result.get(result.size() - 1).equals("EAST")))) {
